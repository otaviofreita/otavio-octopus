...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        private void Awake()
        {
            // Busca a câmera individual do próprio prefab do jogador (necessário para Split-Screen)
            Camera localCam = GetComponentInChildren<Camera>();
            if (localCam != null)
            {
                _mainCamera = localCam.gameObject;
            }
            else
            {
        ...
        private void CameraRotation()
        {
            ...
            // 1. Rotação manual via analógico/mouse (se houver input de look)
            ...
            // 2. Rotação automática ao andar pelo teclado (WASD / Setas)
            else if (_input.move != Vector2.zero && !LockCameraPosition)
            {
                // Faz a orientação do CameraTarget girar suavemente para acompanhar as costas do robô enquanto ele se move
                _cinemachineTargetYaw = Mathf.LerpAngle(_cinemachineTargetYaw, transform.eulerAngles.y, Time.deltaTime * 5.0f);
            }
        ...
        private void Move()
        {
            ...
            {
                // Se não houver referência de câmera válida, usa a rotação mundial padrão
                float cameraYaw = (_mainCamera != null) ? _mainCamera.transform.eulerAngles.y : 0.0f;
